...
struct render_settings {
	...
	explicit render_settings(const unsigned img_w, const unsigned samples = 100, const unsigned temp = 10, const double aspect = 16.0 / 9.0) :
		...
		for (int n = 0; n < number_of_temp; n++) {
			...
			for (int j = (int)image_height-1; j>=0; --j) {
				...
				for (unsigned i = 0; i < image_width; ++i) {
					...
					#pragma omp parallel for //default(none) shared(i, j, pixel_color, ray_color, scn)
